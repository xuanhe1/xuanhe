				...
				'<li>'+
					...
					'<div class="list_none">'+
						'<div>'+
							...
							'</div>'+
							'<div class="none_bottom">'+
								'<p>'+list[i].title+'</p>'+
								'<div class="button11"><a href="./pages/product.html">MORE</a></div>'+
							'</div>'+
						'</div>'+
					'</div>'+
				'</li>'
			).appendTo('.listMenu ul')
		}
	}
})
window.onload=function(){
	...
	function slide(num){
		...
		if(num == 3){
			...
			$('.news-left .img1,.news-left .img2').addClass('animated fadeInLeft delay-1s');
			$('.news-left .img3,.news-left .img4').addClass('animated fadeInUp delay-1s');
			...
			$('.news-left>.button11').addClass('animated fadeIn delay-2s');
			$('.mask').addClass('oMask');
		}
		//联系我们动画
		if(num == 4){
